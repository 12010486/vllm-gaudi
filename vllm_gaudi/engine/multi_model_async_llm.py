...
class MultiModelAsyncLLM:
   ...
   def get_all_vllm_configs(self) -> Dict[str, VllmConfig]:
    ...
    async def switch_model(
        ...
        2. Sleep current model (move to CPU)
        3. Unload current model weights (worker only)
    ...
    async def _unload_model_executor(self) -> None:
        ...
        This releases worker-side model memory without shutting down the engine.
        """
        try:
            await self._engine.collective_rpc(
                method="unload_model",
    ...
    async def encode(
        ...
        **kwargs,
        ...
        async for output in self._engine.encode(
            prompt, pooling_params, request_id, **kwargs
        ):
            yield output
